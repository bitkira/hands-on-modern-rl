...
- 理解为什么 `LunarLander-v3` 比 MountainCar 更适合作为 DQN 的第二个学习案例。
...
上一节我们已经在 CartPole 上跑通了 DQN。CartPole 很适合入门，因为奖励密集、动作只有左右两个，训练曲线很容易变好。但如果一直停在 CartPole，DQN 看起来会过于轻松：好像只要写出 Q 网络、经验回放和目标网络，任何控制任务都能自然学会。
...
LunarLander 的好处在于：它比 CartPole 难，但不会像 MountainCar 那样把最小 DQN 长时间按在失败区间。你能在较短训练中看到策略从随机喷火，逐渐变成“至少知道要减速和调姿态”。
...
```text
timesteps=  5000 eval10_mean=   -14.4 std= 142.5
timesteps= 10000 eval10_mean=  -133.0 std=  19.6
timesteps= 20000 eval10_mean=   -93.3 std=  70.2
timesteps= 40000 eval10_mean=  -108.0 std=  36.7
timesteps= 80000 eval10_mean=   -67.5 std=  41.1
```
...
**第三，短训练不是最终性能。** 如果目标是稳定解决 LunarLander，通常要更长训练、更仔细的超参数、更大的评估集，甚至换 Double DQN、Dueling DQN 或 PPO。这里的目的更朴素：用一个比 CartPole 更真实、又比 MountainCar 更友好的任务，看见 DQN 的学习信号确实能起来。
...
因此，本节想强调的不是“DQN 一定很强”，而是：**例子要和教学目标匹配**。如果目标是讲稀疏奖励失败，MountainCar 很好；如果目标是让学习者在 CartPole 之后继续看到 DQN 的训练效果，LunarLander 更合适。

## 本节收获

- `LunarLander-v3` 是 DQN 的好进阶案例：状态连续、动作离散、奖励比 CartPole 更复杂。
- 随机策略平均回报约 -210，短训练 DQN 可以明显超过这个基线，但仍可能波动。
- 经验回放和目标网络在 LunarLander 中不只是“代码结构”，而是在噪声较大的控制任务中稳定训练的关键。
- MountainCar 更适合讲稀疏奖励和探索失败；LunarLander 更适合讲 DQN 从玩具任务走向更真实控制任务。
