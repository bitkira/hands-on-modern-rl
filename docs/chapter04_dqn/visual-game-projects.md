# 4.6 项目：深度 Q 网络实战——从 LunarLander 到视觉游戏

前面我们理清了 DQN 的理论框架和三个核心组件。现在把它变成代码，并一步步迁移到更复杂的环境。这一页按难度递进排列四个项目：LunarLander 是低维控制入口，Atari 引入像素输入，ViZDoom 引入部分可观测，宝可梦则是稀疏奖励和长时规划的终极挑战。它们不是四套互不相关的算法，而是同一套 DQN 思维在不同环境压力下的四次放大。

## 本节导读

**核心内容**

- 用 LunarLander 观察 DQN 的每个组件（Q-Network、经验回放、目标网络、$\varepsilon$-贪婪）如何把策略从随机控制推向更稳定的着陆控制。
...
理论篇里我们已经推导了 Q-Learning 的更新规则，但那个版本是"表格型"的——给每个 $(s,a)$ 单独存一个数。你可能还记得，第 3 章我们学过 **$Q(s,a)$ 是动作价值函数**，回答"在状态 $s$ 下做动作 $a$，未来一共能拿多少分"。当状态从 LunarLander 的 8 维连续向量膨胀到 Atari 的 28,000 维像素，表格就彻底存不下了，必须用神经网络来近似 $Q(s,a)$。一旦引入神经网络，训练就不再是一个简单的赋值，而是一个真正的优化问题：需要一个损失函数来衡量"网络预测的 Q 值"和"应该有的 Q 值"差多少，然后用梯度下降来缩小这个差距。本节要做的就是把这些公式逐行变成可运行的代码，并观察它们在不同环境压力下如何表现。

## 项目路线

| 项目                | 主要变化                    | 你要关注的问题                                     |
| ------------------- | --------------------------- | -------------------------------------------------- |
| LunarLander         | 低维控制中的 DQN 训练       | Q-Network、经验回放、目标网络如何让策略超过随机？  |
| Atari Pong          | 8 维状态变成像素帧          | CNN、图像预处理、帧堆叠如何接入 DQN？              |
| ViZDoom             | 2D 上帝视角变成 3D 第一人称 | 部分可观测和导航复杂度如何改变训练难度？           |
| stable-retro 宝可梦 | 短局游戏变成长时程 RPG      | 稀疏奖励、动作组合和长期规划为什么会压垮朴素 DQN？ |

## 4.6.1 动手：用深度 Q 网络玩 LunarLander

我们选择 LunarLander 作为本章的低维控制项目。它仍然是离散动作任务，适合 DQN；但比入门平衡任务多了位置、速度、角度、支架接触等状态信息，奖励也更接近真实控制问题。这样做的好处是：学习者不会只看到一个过于容易满分的玩具环境，而是能看见 DQN 在更有噪声、更有权衡的任务中如何逐步超过随机策略。

为什么不是一上来就用 Atari？Atari 需要图像预处理、帧堆叠和 CNN，这些工程细节会分散注意力。LunarLander 的输入是 8 维向量，一个简单的 MLP 就能处理，让我们把精力集中在 DQN 算法本身。等理解了 LunarLander 上的 DQN，后面迁移到 Atari 主要就是换网络结构和预处理流程。

### LunarLander 长什么样？

一艘登月舱从空中下降，目标是在两个旗帜之间尽量平稳地落地。你可以选择不喷、开左侧喷口、开主发动机、开右侧喷口。主发动机能减速，但会耗油；侧喷口能调姿态，但用错方向会越调越歪。

![LunarLander-v3 环境：控制登月舱平稳降落在两个旗帜之间](./images/lunarlander.gif)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>LunarLander-v3 环境动画。来源：<a href="https://gymnasium.farama.org/environments/box2d/lunar_lander/" target="_blank" rel="noopener noreferrer">Gymnasium (Farama Foundation)</a></em>
</div>

LunarLander 的状态有 8 个维度：

| 状态分量             | 含义                     | 直觉                     |
| -------------------- | ------------------------ | ------------------------ |
| `x, y`               | 飞船相对着陆区的位置     | 离中心远不远，高度多少？ |
| `vx, vy`             | 水平和垂直速度           | 是不是下降太快？         |
| `angle, angular_vel` | 倾斜角和角速度           | 姿态有没有歪？           |
| `left_leg`           | 左腿是否接触地面，0 或 1 | 左侧有没有碰到地面？     |
| `right_leg`          | 右腿是否接触地面，0 或 1 | 是否两条腿都接触并落稳？ |

随机策略平均回报通常在 -200 左右，稳定解决环境通常需要平均回报接近 200。短训练未必能解决 LunarLander，但至少应该明显超过随机策略。
...
Q-Network 是一个简单的三层 MLP：8 维输入 → 128 隐藏 → 128 隐藏 → 4 维输出。经验回放池用 `deque` 实现，容量 10000 条——超过容量后旧经验自动淘汰。
...
```
Episode 100/800 | 最近50轮平均奖励: -185.4 | ε: 0.612
Episode 200/800 | 最近50轮平均奖励: -132.7 | ε: 0.311
Episode 300/800 | 最近50轮平均奖励:  -92.8 | ε: 0.142
Episode 500/800 | 最近50轮平均奖励:  -55.6 | ε: 0.035
Episode 800/800 | 最近50轮平均奖励:  -28.4 | ε: 0.012
```

这组数字只是一个典型趋势示意，不是逐行复现实验承诺。LunarLander 的训练波动比入门平衡任务大得多：有时某个阶段突然变好，随后又回落。我们关注的是它是否逐步超过随机策略，而不是每 50 轮都单调上升。
...
测试时关闭探索——直接选 Q 值最大的动作。如果训练有效，平均测试回报应该明显高于随机策略；如果目标是稳定解决环境，还需要更长训练、更稳的超参数，或者换 Double DQN / Dueling DQN。

这个学习过程和第 7 章用 PPO 训练 LunarLander 的现象可以互相对照——只是这里你能看到 DQN 的每一个零件在做什么。经验回放池里的每一条经验长什么样？目标网络多久更新一次？Q 值是怎么从随机噪声变成有意义的动作偏好？这些都不再是黑盒。
...
![DQN 在多种 Atari 游戏上的表现对比（来源：Mnih et al. 2015, Figure 3）](./images/pong-screenshot.jpg)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>DQN 在多种 Atari 游戏上的学习曲线与人类对比。来源：Mnih et al. 2015, <a href="https://www.nature.com/articles/nature14236" target="_blank" rel="noopener noreferrer">Nature 518, 529–533</a>, Figure 3</em>
</div>

上一节我们在 LunarLander 上用 MLP 观察了 DQN——8 个数字输入，一个简单网络即可处理。但真正让 DQN 名声大噪的，是它处理像素级输入的能力。这一节，我们跨越从 8 维向量到 28,000 维像素帧的鸿沟，用 CNN-DQN 学玩 Atari Pong。

### 从 LunarLander 到 Atari：什么变了？

|            | LunarLander   | Atari Pong                         |
| ---------- | ------------- | ---------------------------------- |
| 输入       | 8 维连续向量  | 84×84×4 像素帧（28,224 维）        |
| 网络结构   | MLP（全连接） | CNN（卷积 + 全连接）               |
| 动作空间   | 4 个离散动作  | 6 个（实际上只有上、下、不动有效） |
| 回放池大小 | 10,000        | 通常 100,000                       |
| 训练时间   | 几分钟（CPU） | 数小时（需要 GPU）                 |
| 预处理     | 无            | 灰度化 + 裁剪 + 缩放 + 帧堆叠      |

最显著的变化是输入维度——从 8 个数字变成超过 28,000 个数字。全连接网络处理不了这么高维的输入，我们需要卷积神经网络（CNN）来从像素中提取有用的特征。
...
DQN 论文中使用 Breakout 游戏的连续帧作为示例，展示了帧堆叠的效果。这里先记住关键点：单帧只能告诉你“球在哪里”，连续帧才能告诉你“球往哪里走”。这也是从 LunarLander 迁移到像素游戏时最重要的表示变化。

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>Breakout 的连续游戏帧。来源：<a href="https://danieltakeshi.github.io/2016/12/29/going-deep-with-dqn/" target="_blank" rel="noopener noreferrer">Daniel Takeshi Blog</a></em>
</div>
...
和 LunarLander 的 DQN 相比，关键差异在于：网络从 MLP 换成 CNN；回放池通常继续扩大；新增了奖励裁剪（统一不同游戏的奖励尺度）和梯度裁剪（防止 CNN 梯度爆炸）；训练最好用 GPU，CPU 上会很慢。

![经验回放缓冲区：存储历史转移并随机采样，打破时间相关性](./images/experience-replay.png)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>经验回放缓冲区示意图：环形存储、随机采样。来源：<a href="https://next.gr" target="_blank" rel="noopener noreferrer">next.gr</a></em>
</div>
...
![DQN 在 Atari 游戏上的最终表现：与人类和专业方法对比（来源：Mnih et al. 2015, Figure 4）](./images/dqn-pong-results.jpg)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>DQN 在多款 Atari 游戏上的得分与人类对比。来源：Mnih et al. 2015, <a href="https://www.nature.com/articles/nature14236" target="_blank" rel="noopener noreferrer">Nature 518, 529–533</a>, Figure 4</em>
</div>

DQN 在多种 Atari 游戏上的训练曲线（来源：Mnih et al. 2015, Figure 2）：

![DQN 在 Atari 游戏上的得分随训练步数的变化](./images/dqn-scores.jpg)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>DQN 在 Atari 游戏上的训练曲线。来源：Mnih et al. 2015, <a href="https://www.nature.com/articles/nature14236" target="_blank" rel="noopener noreferrer">Nature 518, 529–533</a>, Figure 2</em>
</div>
...
![CNN 第一层卷积滤波器可视化：训练后自动分化为边缘检测、形状识别等特征检测器](./images/cnn-filters.png)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>CNN 卷积滤波器可视化示例。来源：<a href="https://arxiv.org/abs/2404.18326" target="_blank" rel="noopener noreferrer">arXiv:2404.18326</a></em>
</div>
...
显著性热力图（Saliency Map）将网络对每个像素的敏感度可视化——高亮区域就是网络"在看"的地方。研究表明，训练好的 DQN 最关注的是球的位置和自己的球拍位置，和人类打 Pong 时的关注点完全一致。背景的黑色区域几乎被完全忽略。网络通过训练自动学会了"看哪里"。
...
Atari DQN 比 LunarLander DQN 多了几个工程技巧，每一个都解决了一个具体问题：
...
**更大的回放池**：Atari 的状态空间远大于 LunarLander，需要更多样化的经验。通常使用 10 万到 100 万条经验。

**梯度裁剪**：CNN 的高维参数空间容易出现梯度爆炸。`clip_grad_norm_` 把梯度 L2 范数限制在 10 以内，防止一步更新毁掉一切。

**目标网络更新频率**：Atari 通常每 10,000 步更新一次，比低维控制任务更慢，因为状态空间更大，目标网络需要更长时间保持稳定。

<details>
<summary>思考题：为什么帧堆叠选 4 帧而不是 2 帧或 8 帧？</summary>

4 帧是一个经验性的平衡点。2 帧太少——只能看到一个时间步的位移，方向判断不够准确。8 帧太多——覆盖的时间窗口太长，早期的帧对当前决策没什么帮助，反而增加了输入维度和计算量。4 帧在 Pong 中大约覆盖 0.27 秒的游戏时间（4 帧堆叠 × frame_skip=4 = 16 原始帧 ÷ 60fps），足以推断球的运动方向和速度。
...
回顾一下我们走过的路：LunarLander 输入 8 个数字，Atari 输入 2D 像素帧，ViZDoom 则是 3D 第一人称视角——同样的 DQN 算法，但环境复杂度再次跃升。
...
![ViZDoom Deadly Corridor 场景：第一人称视角下的 3D 走廊，你只能看到眼前的画面，身后和转角完全未知](./images/vizdoom-deadly-corridor.png)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>ViZDoom 第一人称视角游戏画面。来源：<a href="https://arxiv.org/abs/1809.03470" target="_blank" rel="noopener noreferrer">Kempka et al., arXiv:1809.03470</a></em>
</div>
...
走完 LunarLander、Atari、ViZDoom 三个实战，让我们对比一下 DQN 在不同复杂度环境中的表现：

|            | LunarLander     | Atari Pong                   | ViZDoom                  |
| ---------- | --------------- | ---------------------------- | ------------------------ |
| 状态维度   | 8               | 28,224                       | 28,224（但信息密度更低） |
| 网络       | MLP（2 层 128） | CNN（3 层卷积 + 2 层全连接） | 同 Atari                 |
| 训练时间   | 几分钟（CPU）   | 数小时（GPU）                | 数小时（GPU）            |
| 可观测性   | 完全可观测      | 完全可观测                   | 部分可观测               |
| DQN 的瓶颈 | 探索和稳定性    | CNN 特征提取                 | 部分可观测性 + 导航      |
...
LunarLander 的状态是 8 个数字，Atari 的状态是 84×84 像素，ViZDoom 的状态是 3D 第一人称画面。现在，我们要面对一种全新的挑战——**一个需要长期规划、信息不完全、决策链长达数千步的游戏：宝可梦**。
...
```
LunarLander → 8 维向量，MLP，几十分钟以内可做短训练
