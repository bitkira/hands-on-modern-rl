...
前面分别看过熵、交叉熵和 KL 散度。它们其实有一个重要关系：
...
这个目标的直觉是：奖励项把模型往高分回答推，KL 项像安全绳，把模型拉回参考模型附近。

如果 $\beta$ 太小，模型容易为了奖励过度偏移，出现 reward hacking；如果 $\beta$ 太大，模型几乎不敢改变，学习效果弱。

## 进阶公式：DPO 的对数概率比

DPO 不显式训练奖励模型再跑 PPO，而是直接用偏好数据优化策略。它的关键量是新策略相对参考策略的对数概率比：
