...
上一章我们深入了 DPO 的理论与实践，看到它通过数学变换绕过了 RM。但 DPO 是 offline 方法——它只能从固定的偏好数据集中学习，不能在线探索。GRPO 的入口完全不一样：**它不是先看一对 chosen / rejected，而是让模型对同一道题一次写出多个回答，然后在这一组回答内部比较谁更好**。

可以先把 GRPO 想成一场小型同题竞赛：
...
这一节我们就用 GRPO 在线训练一个模型做数学推理，亲眼看看"干掉 Critic"之后训练过程是什么样的，然后深入理解为什么组内归一化能替代 Critic。
...
GRPO 不是从天上掉下来的，但它的第一感觉也不应该是“又一条从 RL 到 PPO 的流程图”。更适合的入口是：**同一个 prompt 下采样一组回答，用组内平均水平当基线，再让比平均好的回答变多、比平均差的回答变少**。

为了把这件事和 PPO 对上号，再把语言模型重新放回强化学习术语里：
...
所以 GRPO 和 PPO 的关系不要记成“完整复制 PPO”，而要记成这张对照卡：
...
这里也顺手澄清一个常见误会：**GRPO 不是一个新的模型，也不只是“组内归一化”这个公式。GRPO 是一种在线训练策略模型的方法。**
