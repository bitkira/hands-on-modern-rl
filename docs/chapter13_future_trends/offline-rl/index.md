...
![Offline RL Paradigm](./images/offline_rl.png)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>图 1：Offline RL 范式。不同于在线 RL（左）需要不断和环境交互试错，Offline RL（右）完全依赖于预先收集好的静态数据集（如人类专家演示、随机探索数据、或是弱 AI 产生的次优数据）来学习最优策略，期间与真实环境零交互。来源：<a href="https://arxiv.org/abs/2005.01643" target="_blank" rel="noopener noreferrer">Offline RL Tutorial</a></em>
</div>
...
### CQL：保守 Q 学习 (Conservative Q-Learning)
...
### IQL：隐式 Q 学习 (Implicit Q-Learning)
...
![Decision Transformer Architecture](./images/decision_transformer.png)

<div style="text-align: center; font-size: 0.9em; color: var(--vp-c-text-2); margin-top: -10px; margin-bottom: 20px;">
  <em>图 2：Decision Transformer 的核心架构。它将强化学习的回报（Return-to-go）、状态（State）和动作（Action）串联成一个长序列，然后使用因果 Transformer（如 GPT 模型）预测未来的动作，完全抛弃了传统的 TD 学习框架。来源：<a href="https://commons.wikimedia.org/wiki/File:Decision_Transformer_Architecture.png" target="_blank" rel="noopener noreferrer">Wikimedia Commons</a></em>
</div>
