...
::: tip 阅读路径
如果后面的公式看不懂，先跳回 [文章开头](#article-start) 或 [先用人话看一遍](#intuition-first)。公式段落下面的“展开”下拉框是慢速解释，第一遍可以先不打开。
:::
...
数学上，Model-Free 并不是“不存在环境转移”，而是 **不去显式学习这个转移模型**。真实环境仍然有一个转移规律：
...
::: details 展开：这行轨迹公式怎么读？
可以把 `tau` 理解成一整段经验录像：第 0 步看到 `s_0`，做动作 `a_0`，拿到奖励 `r_0`，进入下一个状态 `s_1`，然后继续。

`s_{t+1} ~ p(...)` 的意思是：下一状态由真实环境或仿真器产生。智能体只是观察结果，并没有把这个环境规律显式学成一个 `world_model`。

`a_t ~ pi_theta(...)` 的意思是：动作来自当前策略。`theta` 是策略网络参数，训练 RL 的过程就是不断改这些参数。

[回到人话版](#intuition-first) / [返回文章开头](#article-start)
:::
...
::: details 展开：为什么这个目标就是“尽量拿高回报”？
这行公式可以拆成三层：

1. 里面的求和表示“一条轨迹从头到尾拿到的总奖励”。
2. `gamma` 是折扣因子，越靠后的奖励权重越小。
3. 外面的期望表示：策略有随机性，环境也可能有随机性，所以我们优化的是很多次交互的平均回报。

所以 `J_MF(theta)` 就是“当前策略平均能拿多少分”。Model-Free 方法直接让这个分数变大。
:::
...
::: details 展开：策略梯度这行公式在说什么？
这一行不用一开始就推导，只要知道它在回答一个问题：哪个动作更应该被策略多选？

`log pi_theta(a_t | s_t)` 表示策略在状态 `s_t` 下选择动作 `a_t` 的概率。它前面的梯度表示“如果这个动作值得鼓励，应该怎样改策略参数，才能让它以后更常出现”。

`A_t` 是优势函数。它大于 0，说明这个动作比平均水平好；小于 0，说明这个动作不值得鼓励。PPO、A2C 这类算法都在这个思想上做稳定化。
:::

如果走价值函数路线，例如 DQN、SAC、TD3，核心是用真实采样到的下一状态做 TD bootstrapping：
...
::: details 展开：TD 目标和损失函数怎么读？
`y_t` 是一个临时目标：当前奖励 `r_t`，加上“下一状态还能继续拿到的价值”。如果 episode 已经结束，`done=1`，后面那项会被消掉。

`Q_theta(s,a)` 是当前 critic 对“在状态 `s` 做动作 `a` 有多好”的估计。训练 critic 时，就是让它靠近 `y_t`。

这里最重要的是数据来源：这个下一状态来自真实环境或仿真器记录下来的 replay buffer，不是神经网络世界模型生成出来的。
:::
...
这里的这个带帽子的 `p` 是智能体学出来的环境近似模型。它可以预测下一状态、奖励、终止概率，或者在更现代的做法中，预测潜空间里的未来表征。

::: details 展开：为什么要写成带帽子的 p？
没有帽子的 `p` 表示真实环境规律，智能体通常不能直接读到它。

带帽子的 `p_hat` 表示“我用数据学出来的近似版本”。下标 `phi` 是世界模型的参数，就像策略网络有参数 `theta` 一样。

因此 Model-Based 的关键多了一步：先用真实数据训练 `p_hat_phi`，再用它来想象未来。
:::

然后 MBRL 可以用这个学出来的模型做规划：
...
::: details 展开：规划公式怎么读？
`argmax` 的意思是“在很多候选动作序列里，找一个模型预测回报最高的”。

`H` 是规划视野。比如 `H=15`，就是先在模型里看未来 15 步。

`hat{s}` 和 `hat{r}` 上面的帽子表示：这些状态和奖励不是环境真实给的，而是世界模型预测出来的。MPC 每次只执行第一个动作，然后用新的真实观测重新规划，避免一次性相信太长的预测。
:::
...
最后只执行 $a_0^\*$，拿到真实新状态后重新规划。这就是最小版的 Model Predictive Control。
