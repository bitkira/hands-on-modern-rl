...
如果 $\hat{A}_i>0$，最小化 loss 会提高这条轨迹里每一步动作的 log probability；如果 $\hat{A}_i<0$，就会降低这些动作的 log probability。
...
clip 的作用是限制一步更新太猛。它不希望新模型突然把某些去噪动作概率放大很多倍，因为这样很容易破坏原本的生成分布。放到 Diffusion 里理解，就是不要让一次 reward 更新把去噪分布从原始模型附近推得太远；KL 正则和 ratio clipping 都是在控制这件事[^ppo][^dpok]。
